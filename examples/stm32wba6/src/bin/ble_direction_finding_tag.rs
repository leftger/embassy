                    ...
                    BleEvent::Cte(cte_event) => {
                        match cte_event {
                            ...
                            CteEvent::CteRequestFailed(f) => {
                                warn!("CTE Request Failed: conn=0x{:04X} status=0x{:02X}", f.conn_handle, f.status);
                            }
                        }
                        None
                    }
                    ...
                    if let Some(gap_event) = ble.process_event(event) {
                        match gap_event {
                            GapEvent::Connected(conn) => {
                                ...
                                match ble.le_set_connection_cte_transmit_parameters(conn.handle, CTE_TYPES_AOA, &ANTENNA_IDS) {
                                    Ok(()) => {
                                        match ble.le_connection_cte_response_enable(conn.handle, true) {
